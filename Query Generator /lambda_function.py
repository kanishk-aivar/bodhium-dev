...
def get_product_ids_from_rds(products: List[dict]) -> Dict[str, int]:
    """Insert new products (if needed) and return a map of composite key → product_id."""
            ...
            with conn.cursor(row_factory=dict_row) as cur:
                for i, prod in enumerate(products):
                    ...
                    cur.execute(
                        ...
                        SELECT product_id
                        FROM products
                        WHERE product_data->>'name' ILIKE %s
                ...
                for i in range(0, len(queries), batch_size):
                    ...
                    for query_data in batch:
                        try:
                            ...
                            if result:
                                ids.append(result[0])
                                print(f"[QUERIES] Inserted query with ID: {result[0]}")
                        except Exception as e:
...
def lambda_handler(event: Dict[str, Any], context: Any):
    ...
    print(f"[HANDLER] Generated job_id: {job_id}")
    ...
    try:
        ...
        # ── Process products ───────────────────────────────────────────────────
        print(f"[HANDLER] Processing products...")
        ...
        for i, prod in enumerate(products):
            ...
            for q in questions["product_questions"]:
                all_queries.append((prod_id, q, "generated"))
