...
AUTH = 'brd-customer-hl_96d726f6-zone-scraping_browser1:0qo7lvdjft3a'
SBR_WS_CDP = f'wss://{AUTH}@brd.superproxy.io:9222'


# Local output helpers
def _get_local_screenshots_dir() -> str:
    """Return absolute path to `result-bright/screenshots` and ensure it exists."""
    project_root = os.path.abspath(os.path.dirname(__file__))
    screenshots_dir = os.path.join(project_root, 'result-bright', 'screenshots')
    try:
        os.makedirs(screenshots_dir, exist_ok=True)
    except Exception as e:
        print(f"Failed to ensure screenshots directory exists: {e}")
    return screenshots_dir


def save_local_screenshot_from_src(source_path: str, timestamp: str, prefix: str = 'chatgpt_response') -> str:
    """Copy an existing screenshot file to `result-bright/screenshots` and return the new path."""
    try:
        if not source_path or not os.path.exists(source_path):
            return ''
        dest_dir = _get_local_screenshots_dir()
        dest_path = os.path.join(dest_dir, f"{prefix}_{timestamp}.png")
        shutil.copyfile(source_path, dest_path)
        print(f"📁 Local screenshot saved: {dest_path}")
        return dest_path
    except Exception as e:
        print(f"Failed to save local screenshot: {e}")
        return ''
...
def upload_screenshot_to_s3(screenshot_path, bucket_name, s3_path, timestamp):
    """Upload screenshot to S3"""
    try:
        s3_client = boto3.client('s3')
        screenshot_filename = f"chatgpt_response_{timestamp}.png"
        screenshot_s3_key = f"{s3_path}/screenshots/{screenshot_filename}"
...
def upload_processed_data_to_s3(processed_data, bucket_name, s3_path, timestamp):
    """Upload processed data to S3"""
    try:
        s3_client = boto3.client('s3')
        data_filename = f"chatgpt_processed_{timestamp}.json"
        data_s3_key = f"{s3_path}/processed/{data_filename}"
...
def upload_markdown_to_s3(markdown_content, bucket_name, s3_path, timestamp):
    ...
    try:
        s3_client = boto3.client('s3')
        markdown_filename = f"chatgpt_response_{timestamp}.md"
        markdown_s3_key = f"{s3_path}/markdown/{markdown_filename}"
...
async def automate_chatgpt_bright_data(query="top 5 mobile phone brands in india"):
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            save_local_screenshot_from_src(screenshot_path, timestamp, prefix='chatgpt_response')
            ...
            html_s3_key = upload_html_to_s3(full_html_content, S3_BUCKET, S3_PATH, timestamp)
            screenshot_s3_key = upload_screenshot_to_s3(screenshot_path, S3_BUCKET, S3_PATH, timestamp)
            markdown_s3_key = upload_markdown_to_s3(formatted_markdown, S3_BUCKET, S3_PATH, timestamp)
            ...
            processed_s3_key = upload_processed_data_to_s3(response_data, S3_BUCKET, S3_PATH, timestamp)
            if processed_s3_key:
            ...
            try:
                ...
                debug_dir = _get_local_screenshots_dir()
                debug_path = os.path.join(debug_dir, f"error_{ts_err}.png")
...
def create_llm_task(job_id, query_id, llm_model_name="Chatgpt"):
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Extract job_id and query_id from event
        job_id = event.get('job_id', str(uuid.uuid4()))
        query_id = event.get('query_id', str(uuid.uuid4()))
        
        print(f"📊 Job ID: {job_id}, Query ID: {query_id}")
        
        # Create LLM task in database
        try:
            task_id = create_llm_task(job_id, query_id, "ChatGPT")
...
if __name__ == '__main__':
    test_event = {
        'query': 'top 5 mobile phone brands in india'
    }
